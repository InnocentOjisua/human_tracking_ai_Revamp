...
class EyeTracker:
    """Tracks eye positions and computes fixation, saccade, stability, and blink metrics."""
    ...
    def __init__(self, max_len=1800, blink_threshold=0.25, blink_window_sec=2.0, ema_alpha=0.3):
        ...
        self.blink_times = deque(maxlen=300)  # store recent blink timestamps
    ...
    def update(self, left_iris, right_iris, left_eye_landmarks=None, right_eye_landmarks=None):
        """Update iris positions and track blinks."""
        if left_iris is None or right_iris is None:
            return

        # Ensure proper shape (2D points)
    ...
    def _compute_blink_rate(self):
        """Compute blink rate (blinks per minute) using recent timestamps with EMA smoothing."""
        now = time.time()
    ...
    def compute_metrics(self):
        """Compute normalized metrics (0-100) for fixation, saccades, stability, and blink rate."""
        if len(self.left_eye_buf) < 2 or len(self.right_eye_buf) < 2:
            return self.fixation_ema, self.saccades_ema, self.stability_ema, self._compute_blink_rate()
